...
import { firebaseInit, login, alertService , signup, writeUserData} from '../helpers';
  ...
  submit() {
    if (this.state.email && this.state.password){
      ...
      method(this.state.email, this.state.password)
        .then((result) => {
          // firebase has no service to add custom field so updating userinfo with his/her id.
          result.updateProfile({displayName: this.state.username});
        })
        .catch((error) => {
      ...
      <View style={styles.root}>
        <View style={styles.LoginScreenContainer}>
          ...
          {this.state.screenName !== 'Login' &&
            ...
            <FormLabel fontFamily="catamaran">Username</FormLabel>
            <FormInput
